...
const UserProfile = () => {
    ...
    const handleUpdate = async (e) => {
        ...
        setLoading(true);
        try {
            const response = await api.put(`/paciente/${patient.id}`, formData);
    ...
    const handleDelete = async () => {
        if (!window.confirm('Tem certeza que deseja excluir seu cadastro de paciente? Esta ação não pode ser desfeita.')) {
            return;
        }

        setLoading(true);
        try {
            const response = await api.delete(`/paciente/${patient.id}`);
            const successMessage = response.data.message || "Cadastro de paciente excluído com sucesso!";
            showToast(successMessage, "success");
    ...
    const handleCancel = () => {
        setFormData({
            ...
            phone: patient.phone || '',
            address: {
                publicPlace: patient.address?.publicPlace || '',
    ...
    return (
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                <h2 style={{ margin: 0, color: 'var(--primary-color)' }}>Minhas Informações</h2>
